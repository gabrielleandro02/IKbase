...
app.get("/dbtopico/:tecnologia", async (req, res)=>{
    let result= await db.getTopicoByTag(req.query.tecnologia)
    res.send(result)
    res.end()
})
​
app.get("/dbtopico/:finalizado&:tecnologia", async (req, res)=>{
    const {finalizado, tecnologia} = req.body;
    let result= await db.getTopicoAbertoByTag(finalizado,tecnologia)
    res.send(result)
    res.end()
})
​
​
app.get("/dbcomentarios/:titulo", async(req, res)=>{
    let result=await db.getComentariosByTopico(req.params.titulo)
    res.send(result)
    res.end()
})
​
